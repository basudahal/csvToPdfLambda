...
function readCSV(filepath) {
  ...
  return readFile(filepath, { encoding: 'utf8' }).then((file) =>
    convertCSVToArray(file, { type: 'array', separator: SEPARATOR })
  );
}
...
module.exports = function convert(inFile, outFile) {
  return readCSV(inFile).then((csvRows) => {
    ...
    if (HAS_HEADERS) {
      const csvHeaders = csvRows
        .splice(0, 1)[0]
        .map((headerName) => ({ text: headerName }));
      pdfStructure.content[0].table.body.push(csvHeaders);
    }
    pdfStructure.content[0].table.body = pdfStructure.content[0].table.body.concat(
      csvRows
    );
